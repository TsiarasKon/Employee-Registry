...
@Component
public class SearchHardTaskStrategy implements SearchTaskStrategy{
    @Autowired
    ...
    public List<Task> execute(Long assignedEmployeesNumber, Iterable<Task> tasks) throws InvalidIdException {
        ...
        for (Task task : tasks) {
            if (mapper.mapTaskId(task).getDifficulty().toLowerCase().equals("hard") && assignedEmployeesNumber==null)
                allTasks.add(task);
            else if(mapper.mapTaskId(task).getDifficulty().toLowerCase().equals("hard") && assignedEmployeesNumber==mapper.mapTaskId(task).getAssignedEmployees().size())
                allTasks.add(task);
        }
        return allTasks;
    }
